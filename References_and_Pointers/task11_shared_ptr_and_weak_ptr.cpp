/*The cloud system needs to handle shared resources like connection pools and monitoring services that multiple components can access simultaneously. You'll implement shared ownership with reference counting and use weak_ptr to prevent circular dependencies.

🔍 Practice
    Complete the shared resource management system using the provided code.
    Test and modify it by creating additional components and observers.
Reflect using prompts:
    How does reference counting ensure resource lifetime management?
    What happens when you access an expired weak_ptr?*/
...
public:
    ConnectionPool(const string& name, int maxConn) 
        ...
        cout << "Connection pool '" << poolName << "' created (max: " << maxConnections << ")" << endl;
    }    
    ~ConnectionPool() {
    ...
    bool getConnection() {
        if (activeConnections < maxConnections) {
            activeConnections++;
            cout << "Connection acquired from " << poolName << " (" << activeConnections 
...
class ServiceComponent:  enable_shared_from_this<ServiceComponent> {
private:
...
public:
    ServiceComponent(const string& name, shared_ptr<ConnectionPool> connectionPool) 
    ...
    shared_ptr<ServiceComponent> getShared() {
        return shared_from_this();
    }    
private:
    // This allows shared_from_this to work
    class EnableSharedFromThis : public std::enable_shared_from_this<ServiceComponent> {};
};
// Fix the shared_from_this issue by using enable_shared_from_this
class ModernServiceComponent : public std::enable_shared_from_this<ModernServiceComponent> {
private:
    string componentName;
    shared_ptr<ConnectionPool> pool;
...
public:
    ModernServiceComponent(const string& name, shared_ptr<ConnectionPool> connectionPool) 
        : componentName(name), pool(connectionPool) {
        cout << "Modern service component '" << componentName << "' created" << endl;
    }    
    ~ModernServiceComponent() {
        cout << "Modern service component '" << componentName << "' destroyed" << endl;
    }    
    void addChild(shared_ptr<ModernServiceComponent> child) {
        child->parent = shared_from_this();
        children.push_back(child);
        cout << "Added child to " << componentName << " (parent refs: " << shared_from_this().use_count() << ")" << endl;
    }    
    void usePool() {
        if (pool) {
...
int main() {
    ...
    auto authService = make_shared<ModernServiceComponent>("AuthService", sharedPool);
    auto userService = make_shared<ModernServiceComponent>("UserService", sharedPool);
    auto dataService = make_shared<ModernServiceComponent>("DataService", sharedPool);    
    cout << "Pool reference count after creating components: " << sharedPool.use_count() << endl;    
    ...
    cout << "\n--- Component Information ---" << endl;
    authService->displayInfo();
    userService->displayInfo();
    dataService->displayInfo();    
    ...
    cout << "Pool observer expired? " << (poolObserver.expired() ? "Yes" : "No") << endl;
    cout << "Component observer expired? " << (componentObserver.expired() ? "Yes" : "No") << endl;    
    // Release some references
    cout << "\n--- Releasing References ---" << endl;
    cout << "Before releasing userService, pool refs: " << sharedPool.use_count() << endl;    
    userService.reset();  // Release userService reference
...
/*✅ Success Checklist
Multiple components successfully share the connection pool

Reference counting accurately tracks shared ownership

weak_ptr prevents circular dependencies in the hierarchy
