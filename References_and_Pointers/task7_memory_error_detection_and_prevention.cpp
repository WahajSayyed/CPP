/*The laboratory system needs to handle multiple datasets simultaneously while detecting and preventing common memory errors that could compromise research data integrity.

🔍 Practice
Complete the provided code to achieve comprehensive error detection.
Test and modify it by creating various error scenarios.
Reflect using prompts:
    How does the tracker prevent double allocation of the same dataset?
    What happens when you try to deallocate a dataset that doesn't exist?
*/
#include <iostream>
#include <vector>
#include <string>
...
public:
    ...
    void displayActiveDataSets() {
        ...
        for (DataSet* set : activeSets) {
            ...
            cout << "Dataset: " << set->name << " | Size: " << set->size 
                 << " | Age: " << seconds << "s | Address: " << set->data << endl;
        }
    }    
